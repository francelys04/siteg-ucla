...
public interface IDefensaDAO extends JpaRepository<Defensa, Long> {
	...
	@Query("select defensa from Defensa defensa where defensa.teg in (select teg from Teg teg where teg.tematica in (select tematica from Tematica tematica where tematica.areaInvestigacion=?1))and defensa.fecha between ?2 and ?3")
