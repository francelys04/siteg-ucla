...
public class CCatalogoCalificarDefensa extends CGeneral {
	...
	{
List<Jurado> j = servicioj.buscarTegDeProfesor(ObtenerUsuarioProfesor());
	...
	public void filtrarDatosCatalogo() {
		...
		for (Teg teg : teg1) {
			if (teg
					...
					.toLowerCase()
					.contains(
					txtMostrarFechaCalificar.getValue().toLowerCase())
					
				&&	teg
					.getTematica().getNombre()
					.toLowerCase()
					.contains(
							txtMostrarTematicaCalificar.getValue().toLowerCase())					
