...
/**
 * Controlador asociado a la vista catalogo noticia que permite mostrar las noticias
...
public class CCatalogoNoticia extends CGeneral {
	...
	public void filtrarDatosCatalogo() {
		...
		for (Noticia noticia : noticia1) {
			if (noticia.getNombre().toLowerCase()
					.contains(txtNombreMostrarNoticia.getValue().toLowerCase())
					&& noticia
							.getDescripcion()
							.toLowerCase()
							.contains(
									txtDescripcionMostrarNoticia.getValue()
											.toLowerCase()))
	...
	/**
	 * Metodo que permite obtener el objeto Noticia al realizar el evento doble
	...
	public void mostrarDatosCatalogo() {
		...
		if (noticia.size() == 2) {
			Messagebox
					.show("No puede eliminarse deben haber minimo dos imagenes, registre una y elimine",
