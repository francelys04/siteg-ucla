...
public interface IDefensaDAO extends JpaRepository<Defensa, Long> {
	...
	@Query("select defensa from Defensa defensa where defensa.estatus=?1 and defensa.fecha between ?3 and ?4 and defensa.teg in (select teg from Teg teg where teg.tematica=?2)")
